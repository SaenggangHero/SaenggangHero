...
export default function App() {
  ...
  function toggleChapter(book, chapter) {
    ...
    updatePerson(selectedPerson.id, (person) => {
      ...
      return {
        ...nextPerson,
        checked: { ...nextPerson.checked, [key]: !nextPerson.checked[key] },
      };
    });
  }
  ...
  function toggleWholeBook(book) {
    ...
    updatePerson(selectedPerson.id, (person) => {
      ...
      Array.from({ length: book.chapters }, (_, index) => index + 1).forEach((chapter) => {
        nextChecked[`${book.name}-${chapter}`] = !isAllDone;
      });
      return { ...nextPerson, checked: nextChecked };
    });
  }

  function resetCurrentCheck() {
    if (!selectedPerson) return;
    updatePerson(selectedPerson.id, (person) => ({ ...person, checked: {} }));
  }
  ...
  function personStats(person) {
    return {
      ...
      checkedChapters: checkedCount(person.checked),
    };
  }
  ...
  function addParticipant() {
    ...
    setPeople((prev) => [
      ...prev,
      { id: makeId("person"), name, checked: {}, completions: [], readingDates: [] },
    ]);
    setNewName("");
  }
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        @media (max-width: 420px) { .name-grid { grid-template-columns: 1fr; } .chapter-grid { grid-template-columns: repeat(3, 1fr); } .tab { min-width: 0; flex: 1; } .stat-number { font-size: 50px; min-height: 100px; } }
      `}</style>
                    ...
                    <section className="admin-section">
                      ...
                      <div className="admin-table-wrap">
                        <table className="admin-table">
                          <thead><tr><th>이름</th><th>통독 횟수</th><th>이번주 읽은 횟수</th><th>읽은 장수</th></tr></thead>
                          <tbody>
                            {people.map((person) => {
                              const stats = personStats(person);
                              return <tr key={person.id}><td>{person.name}</td><td>{stats.totalReads}독</td><td>{stats.weeklyReads}회</td><td>{stats.checkedChapters}장</td></tr>;
                            })}
                          </tbody>
                        </table>
                      </div>
